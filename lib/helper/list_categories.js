...
module.exports = function listCategoriesHelper(categories, options) {
  ...
  function urlForCategory(category) {
    return self.url_for(self.page.lang + '/' + category.path.replace(self.config.default.category_dir, self.config.category_dir));
  }
  ...
  function prepareQuery(parent) {
    ...
    return categories.find(query).sort(orderby, order).filter(function(cat) {
      return cat.length && cat.posts.filter(function(post) {
        return post.lang == self.page.lang;
      }).length;
    });
  }
    ...
    prepareQuery(parent).forEach(function(cat, i) {
      ...
      if (showCurrent && self.page) {
        ...
        if (!isCurrent && self.page.base) {
          if (self.page.base.indexOf(cat.path.replace(self.config.default.category_dir, self.config.category_dir)) === 3) {
            isCurrent = true;
          }
        }
      }
      ...
      result += '<a class="' + className + '-list-link' + (isCurrent ? ' current' : '') + '" href="' + urlForCategory(cat) + '">';
      result += transform ? transform(cat.name) : cat.name;
      result += '</a>';

      if (showCount) {
        result += '<span class="' + className + '-list-count">' + cat.posts.filter(function(post) {
          return post.lang == self.page.lang;
        }).length + '</span>';
      }
  ...
  function flatList(level, parent) {
    ...
    prepareQuery(parent).forEach(function(cat, i) {
      ...
      result += '<a class="' + className + '-link" href="' + urlForCategory(cat) + '">';
      result += transform ? transform(cat.name) : cat.name;

      if (showCount) {
        result += '<span class="' + className + '-count">' + cat.posts.filter(function(post) {
          return post.lang == self.page.lang;
        }).length + '</span>';
      }
